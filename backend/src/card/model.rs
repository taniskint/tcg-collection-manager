...
pub fn create_many(
    ...
    for card in cards {
        ...
        conn.execute(
            "INSERT INTO cards (set_id, name, image_url, attributes) VALUES (?1, ?2, ?3, ?4)",
            params![set_id, card.name, card.image_url, attrs_json],
        )
...
pub fn get(conn: &Connection, set_id: i64, card_id: i64) -> Result<Option<Card>, SqliteError> {
    let mut stmt =
        conn.prepare("SELECT id, name, image_url, attributes FROM cards WHERE set_id = ?1 AND id = ?2")?;
    ...
    match rows.next()? {
        Some(row) => Ok(Some(Card {
            ...
            name: row.get(1)?,
            image_url: row.get(2)?,
            attributes: deserialize_attributes(row.get(3)?),
        })),
        None => Ok(None),
    }
}
...
pub fn list(conn: &Connection, set_id: i64) -> Result<Vec<Card>, SqliteError> {
    let mut stmt = conn.prepare("SELECT id, name, image_url, attributes FROM cards WHERE set_id = ?1")?;
